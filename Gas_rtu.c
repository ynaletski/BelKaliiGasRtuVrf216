...
//22.10.2021 YN void GetModuleAout (unsigned char buf_com[],unsigned char num);
...
void main (void)
{
  ...
  for (;;)
  {
    ...
    {
      Prt.nmb_icp++;
      if (Prt.nmb_icp > 8) Prt.nmb_icp=0;
    ...
    if (Port[0].status == 4) /* обработка ответа от модулей или индикатора */
    {
      ...
      {
	      if (icp_pool < 9) ReadFromICP(Prt.nmb_icp);
        else if (Port[0].buf[0]==0x21 &&(icp_pool==16 || icp_pool==17))
	      {
          if (step==0) 
          {
            Display.evt=0;
        ...
        else if (icp_pool == 15 || Display.suspend==1)
    ...
    { /* выдача хост-запроса в Modbus устройство подключенное к СОМ3 */
      ...
      if (Host.flag == 0)
      {
	      ...
	      if ((buf_evt[7] & 0x80) > 0) SendModbus(buf_evt,3);
        else Host.num_pool++;
      }
    }
    if (flg_arc_clr == 1) 
    ...
    { 
      ...
      Config[i].flag=0;
      break;
    }/*расчёт расхода*/ //главная формула
    if (Prt.flg_err == 1) 
    { 
      ViewError();
      Prt.flg_err=0;
    ...
    счётчиков в памяти часов реального времени*/
    {
      ...
      cnt[2]=0;
      for (k=0;k< Max_pnt;k++) SaveParameters(k);
      SaveExpParams();
...
void MyTimer (void)
{
  ...
  if (icp_pool != 0)
  {
      ...
      for (a=0;a<Port[0].index;a++) if (Port[0].buf[a]==Key_termin)
    {
      ...
      Port[0].index=a;
	    ClearCom_1(); 
      if (Prt.nmb_icp<=4) err[Prt.nmb_icp+4]=0;
      break;
    }
  }
...
{
  unsigned char flg_date,flg_time,i,j,k,buf_func[6],flag,mb_err,cr[4],flg_wr;
  unsigned res, value_dt;
  float value,lo_scale,hi_scale; 
  flg_time=flg_date=flg_wr=mb_err=0; 
  for (i=0;i<*cnt_reg;i++)
  {
    flag=0;
    for (k=0;k<mb_page[ind_func][1];k++)
    {
      for (j=0;j<40;j++)
      {
	      X607_ReadFn(0x0a000+0x100*mb_page[ind_func][0]+0x100*k+j*6,6,buf_func);
    ...
    else
    {
      ...
      if (ind_func==2) /*функция с номером 03*/
      {
	      ...
	      if (buf_func[2]==3)  //buf_func[2] В таблице функций модбас -> Номер точки (VRFLink)
	      {
	        switch (buf_func[4])  //buf_func[4] В таблице функций модбас -> Номер параметра (VRFLink)
	        {
	          case 0:value_dt=year; break; 
            case 1:value_dt=month; break;
	          case 2:value_dt=day; break; 
            case 3:value_dt=hour; break;
	          case 4:value_dt=min; break; 
            case 5:value_dt=sec; break;
	        }
	      }
	      ...
	      switch (buf_func[5])
	      {
	        ...
	        case 20: 
            data[num]=value_dt/256;
		        data[num+1]=value_dt-data[num]*256; 
          break;
      ...
      if (ind_func==7 && buf_func[2]==2) /*функция с номером 16 доп точка*/
      {
	      ...
	      if (flg_wr>=2)
	      {
	        ConvToFloatVerify(&Expand.dyn[buf_func[4]],cr[0],cr[1],cr[2],cr[3]); 
          flg_wr=0;
	      }
      }

      else if (ind_func==7 && buf_func[2]==3 && buf_func[5]==20)
      {
	      value_dt=data[num]*256+data[num+1];
	      switch (buf_func[4])
	      {
	        case 0:year=value_dt; flg_date=1; break;
	        case 1:month=value_dt; flg_date=1; break;
	        case 2:day=value_dt; flg_date=1; break;
	        case 3:hour=value_dt; flg_time=1; break;
	        case 4:min=value_dt; flg_time=1; break;
	        case 5:sec=value_dt; flg_time=1; break;
	      }
      }
  ...
  *cnt_reg=num;
  ...
  if (flg_time==1) SetTime(hour,min,sec);
  if (flg_date==1) SetDate(year,month,day);
	...
	if (buf[7]>0)
	{
      ...
      case 22:if (buf[7] < Max_icp)
		...
		{  /*тип,формат,имя модуля*/
		  ...
		  case 2:/*GetModuleAout(buf,0);*/break;//22.10.2021 YN
		  case 3:/*GetModuleAout(buf,1);*/break;//22.10.2021 YN
